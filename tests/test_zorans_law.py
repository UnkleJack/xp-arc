...
# ─── Primary Role Occupancy ──────────────────────────────────────────────────

def test_infrastructure_stations_do_not_dilute_pro(pool):
    """PRO counts agents in a role, not pool plumbing that holds a write key.

    The Aboyeur, the Fracture Protocol and the competitive-intel bridge all
    register with the pool purely to obtain an HMAC key. Counting them as
    non-primary agents reported a spurious compression_review over a healthy
    brigade — which is exactly what the first acceptance run did (66.7%).
    """
    pool.register_station('worker_one', 'Worker One', ['url'], is_primary=True)
    pool.register_station('worker_two', 'Worker Two', ['url'], is_primary=True)
    pool.register_station('aboyeur', 'Aboyeur', ['qa'], is_primary=False)
    pool.register_station('fracture_protocol', 'Fracture', ['shard'],
                          is_primary=False)
    pool.register_station('competitive_intel_bridge', 'Bridge',
                          ['competitive_gap'], is_primary=False)

    m = ZoransLaw(pool).measure()

    assert m['active_stations'] == 2
    assert m['primary_role_occupancy'] == 1.0
    assert 'compression_review' not in m['system_state']


def test_genuine_fallback_occupancy_still_triggers_review(pool):
    """The exclusion must not mask a brigade actually running on fallbacks."""
    pool.register_station('worker_one', 'Worker One', ['url'], is_primary=True)
    for i in range(3):
        pool.register_station(f'fallback_{i}', f'Fallback {i}', ['url'],
                              is_primary=False)

    m = ZoransLaw(pool).measure()

    assert m['primary_role_occupancy'] == 0.25
    assert 'compression_review' in m['system_state']
