...
# ─── Closed findings: tests that pin the FIX, not the vulnerability ─────────
#
# RT-04, RT-12 and RT-13 were removed when this suite was salvaged from the
# abandoned PR #4. All three asserted that a vulnerability was PRESENT, so each
# one started failing the moment the vulnerability was fixed — an inverted test
# that goes red on success is worse than no test.
#
# They are replaced below by tests that assert the mitigations hold, so a
# regression re-opens a red build instead of quietly restoring the hole.
#
#   RT-04 -> the Aboyeur had a known default signing key. It now has none and
#            refuses to construct without one.
#   RT-12 -> SQLiteBrokerExecutor.execute_raw() ran arbitrary SQL. The Write
#            Broker was retired by ruling; the module must stay gone.
#   RT-13 -> Zoran's S could be inflated with extreme sla_seconds. SLA is now
#            clamped, and unsigned completions are refused outright.


def test_rt04_closed_aboyeur_has_no_default_signing_key(pool, monkeypatch):
    """A known default signing key lets anyone forge ABOY- signatures."""
    from xp_arc.core.aboyeur import Aboyeur

    monkeypatch.delenv('XP_ARC_ABOYEUR_KEY', raising=False)

    with pytest.raises(RuntimeError, match='XP_ARC_ABOYEUR_KEY'):
        Aboyeur(pool)

    # And no default constant survives anywhere in the module source.
    src = pathlib.Path(
        importlib.import_module('xp_arc.core.aboyeur').__file__).read_text()
    assert 'development-only-change-me' not in src


def test_rt12_closed_broker_execute_raw_is_unreachable():
    """RT-12's real threat was reachability, not the method's existence.

    SQLiteBrokerExecutor.execute_raw() runs arbitrary SQL. That only matters if
    untrusted input can reach it, so what this pins is that NOTHING imports the
    broker: not the entrypoints, not the stations, not the pool.

    NOTE: broker.py and broker_client.py are still present in the tree even
    though the standing ruling retired the Write Broker permanently. They are
    orphaned rather than wired in, so the vulnerability is not reachable — but
    deleting them is a separate scope decision and is still outstanding. When
    that happens, tighten this test to assert the files are gone.
    """
    root = pathlib.Path(__file__).resolve().parent.parent
    importers = []
    for path in root.rglob('*.py'):
        if 'broker' in path.name or '.git' in path.parts:
            continue
        try:
            tree = ast.parse(path.read_text(encoding='utf-8', errors='ignore'))
        except SyntaxError:
            continue
        # Parse real import statements. A substring scan matches this test's
        # own prose and every comment that merely mentions the broker.
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.Import):
                ...
                mods = [node.module or '']
            if any('broker' in (m or '') for m in mods):
                importers.append(str(path.relative_to(root)))
                break

    assert importers == [], (
        f'The retired Write Broker is imported by {importers}. '
        f'execute_raw() runs arbitrary SQL — nothing may route input to it.'
    )


def test_rt13_closed_sla_is_clamped_at_ingestion(pool):
    """Extreme sla_seconds inflated S. Values are now clamped to a sane band."""
    eid = pool.add_entity('url', 'https://rt13-high.test/', sla_seconds=99999)
    assert pool.get_entity(eid)['sla_seconds'] == 3600

    eid_low = pool.add_entity('url', 'https://rt13-low.test/', sla_seconds=-5)
    assert pool.get_entity(eid_low)['sla_seconds'] >= 1


def test_rt13_closed_unsigned_completion_is_refused(pool):
    """The other half of RT-13: you cannot manufacture completed entities.

    RT-13 staged its attack by walking an entity to 'completed' with no Aboyeur
    signature. That path is now closed, which is why the original test could no
    longer even set up its own scenario.
    """
    eid = pool.add_entity('url', 'https://rt13-unsigned.test/')
    writer = pool.station_writer('legacy_local')
    writer.transition_status(eid, 'processing', station='legacy_local')
    writer.transition_status(eid, 'pending_qa')

    assert writer.transition_status(eid, 'completed') is False
    assert pool.get_entity(eid)['status'] != 'completed'
